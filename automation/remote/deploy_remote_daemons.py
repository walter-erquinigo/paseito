...
test "$(jq -r '.features // [.feature] | contains(["changesContextExpansion", "reviewSuggestionsV1", "fileReviewV1"])' "$extract/paseito-daemon/manifest.json")" = true
...
if test -e "$release"; then
  ...
  test "$(jq -r '.features // [.feature] | contains(["changesContextExpansion", "reviewSuggestionsV1", "fileReviewV1"])' "$release/manifest.json")" = true
