...
def validate_provenance(value: Any, release_tag: str, peeled_commit: str) -> dict[str, Any]:
    ...
    if value.get("schemaVersion") in {2, 3}:
        ...
        if value.get("schemaVersion") == 3:
            ...
            manifest_checksum = daemon.get("manifestSha256")
            runtime_checksum = daemon.get("runtimeIntegritySha256")
            daemon_runtime = value.get("daemonRuntime")
            if (
                not isinstance(manifest_checksum, str)
                or not re.fullmatch(r"[0-9a-f]{64}", manifest_checksum)
                or not isinstance(runtime_checksum, str)
                or not re.fullmatch(r"[0-9a-f]{64}", runtime_checksum)
                or daemon_runtime
                != {
                    "manifestSha256": manifest_checksum,
                    "runtimeIntegritySha256": runtime_checksum,
                }
            ):
                raise InstallError("provenance", "daemon runtime provenance is invalid")
    return value
