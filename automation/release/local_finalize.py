...
message and the complete final changed-file list. A failed or uncertain check is a blocker.
"""
...
def validate_result(result: dict[str, Any], files: list[str]) -> str:
...
def finalize_local_changes(repo: Path, state_root: Path) -> str:
    ...
    message = validate_result(result, files)
