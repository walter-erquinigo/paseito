...
class RebasePipelineTests(unittest.TestCase):
    ...
    def test_paseito_has_no_github_actions_or_publication_workflow(self) -> None:
        workflows = Path(__file__).parents[2] / ".github/workflows"
        self.assertFalse((workflows / "paseito-release.yml").exists())
        self.assertFalse((workflows / "paseito-local-status.yml").exists())
    ...
    def test_every_registry_vitest_contract_fails_fast(self) -> None:
        registry = json.loads(
            (Path(__file__).parents[2] / "automation/feature-registry.json").read_text(
                encoding="utf-8"
            )
        )
        vitest_contracts = [
            ...
            for command in feature["contracts"]
            if command.startswith("npx vitest run ")
        ]
        self.assertTrue(vitest_contracts)
        self.assertTrue(all("--bail=1" in command for command in vitest_contracts))

    def test_local_controller_prepares_validated_history_before_semantic_work(self) -> None:
        controller = (Path(__file__).parent / "semantic_sync.py").read_text(encoding="utf-8")
        synchronize = controller[controller.index("def synchronize(") :]
        self.assertLess(
            synchronize.index("prepare_candidate_history"),
            synchronize.index('command(["npm", "ci"]'),
        )

    def test_local_controller_builds_before_branch_push_and_install(self) -> None:
        controller = (Path(__file__).parent / "semantic_sync.py").read_text(encoding="utf-8")
        synchronize = controller[controller.index("def synchronize(") :]
        self.assertLess(
            synchronize.index("focused_verification(candidate)"),
            synchronize.index("build_local_macos_app("),
        )
        self.assertLess(
            synchronize.index("build_local_macos_app("),
            synchronize.index("push_and_install_local("),
        )
        self.assertIn('"PASEO_DESKTOP_SMOKE": "1"', controller)

    def test_controller_has_no_actions_release_or_remote_deployment_path(self) -> None:
        controller = (Path(__file__).parent / "semantic_sync.py").read_text(encoding="utf-8")
        self.assertNotIn('"gh", "workflow"', controller)
        self.assertNotIn('"gh", "release"', controller)
        self.assertNotIn("deploy_remote_daemons.py", controller)
        self.assertNotIn("--remote-restart-approved", controller)
