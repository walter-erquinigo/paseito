...
class RebasePipelineTests(unittest.TestCase):
    ...
    def test_local_controller_validates_normalized_history_before_rebase(self) -> None:
        controller = (Path(__file__).parent / "semantic_sync.py").read_text(encoding="utf-8")
        synchronize = controller[controller.index("def synchronize(") :]
        self.assertLess(
            synchronize.index("validate_normalized_history"),
            synchronize.index("rebase_candidate"),
        )
