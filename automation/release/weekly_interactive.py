...
9. When GitHub Actions is available, build and publish the next Paseito version through the repository's release workflow. Verify the published artifacts and provenance, install the verified macOS application, and deploy registered remote daemons using the repository automation.
10. Before quitting Paseito or restarting its desktop-managed daemon, warn that active agents will be interrupted and ask the human for explicit permission. Never use launchctl submit. Verify the installed bundle version, running GUI bundle path, and daemonNode path before claiming the release is active. Keep this iTerm2 window open on every failure so the human can intervene.
"""
...
def iterm_script(actions_unavailable: bool = False) -> str:
    script = terminal_command(actions_unavailable).replace("\\", "\\\\").replace('"', '\\"')
    return (
        'tell application "iTerm2"\n'
        "activate\n"
        f'create window with default profile command "{script}"\n'
        "end tell"
    )
...
def launch_terminal(actions_unavailable: bool = False) -> int:
    if not ITERM.is_dir():
        ...
        return 1
    apple_script = (
        iterm_script(actions_unavailable)
    )
    return subprocess.run(["/usr/bin/osascript", "-e", apple_script], check=False).returncode
