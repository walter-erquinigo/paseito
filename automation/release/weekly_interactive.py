...
PROMPT = """Maintain Paseito from this checkout. Work interactively and stop for the human whenever a blocker or consequential choice appears.
...
7. Re-run the registry history checks, focused tests, and required repository checks. Build and smoke-test the Apple Silicon macOS app locally. Commit and force-push the final `paseito` branch with `--force-with-lease` only after every check passes.
8. Never dispatch or wait for GitHub Actions, create a release tag, publish provenance or release assets, or deploy remote daemons. GitHub is used only to fetch upstream and push the maintained `paseito` branch.
9. Install the verified local build without quitting Paseito. Before quitting Paseito or restarting its desktop-managed daemon, warn that active agents will be interrupted and ask the human for separate explicit permission. Never use launchctl submit. Verify the installed bundle version, running GUI bundle path, and daemonNode path before claiming the installed release is active. Keep this iTerm2 window open on every failure so the human can intervene.
"""
...
def iterm_launch_arguments() -> list[str]:
    return [
        "/usr/bin/open",
        "-na",
        str(ITERM),
...
def launch_terminal() -> int:
    if not ITERM.is_dir():
        ...
        return 1
    return subprocess.run(iterm_launch_arguments(), check=False).returncode
...
def main() -> int:
    ...
    return run_codex() if args.run else launch_terminal()
