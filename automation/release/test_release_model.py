...
class ReleaseModelTests(unittest.TestCase):
    ...
    def test_new_upstream_increments_matching_source_revision(self) -> None:
        decision = decide_release(
            "v0.3.0",
            "v0.2.5",
            "0.2.5-paseito.7",
            False,
            source_version="0.3.0-paseito.51",
        )
        self.assertTrue(decision.needs_release)
        self.assertEqual(decision.version, "0.3.0-paseito.52")
    ...
    def test_checked_in_revision_ahead_of_provenance_is_releasable(self) -> None:
        decision = decide_release(
            "v0.4.0",
            "v0.4.0",
            "0.4.0-paseito.33",
        ...
        self.assertTrue(decision.needs_release)
        self.assertEqual(decision.version, "0.4.0-paseito.34")

    def test_source_revision_ahead_of_provenance_reserves_the_next_revision(self) -> None:
        decision = decide_release(
            "v0.4.0",
            "v0.4.0",
            "0.4.0-paseito.33",
            False,
            source_version="0.4.0-paseito.34",
        )

        self.assertTrue(decision.needs_release)
        self.assertEqual(decision.version, "0.4.0-paseito.35")

    def test_forced_release_increments_past_source_and_published_revisions(self) -> None:
        decision = decide_release(
            "v0.4.0",
            "v0.4.0",
            "0.4.0-paseito.33",
            True,
            source_version="0.4.0-paseito.51",
        )

        self.assertTrue(decision.needs_release)
        self.assertEqual(decision.version, "0.4.0-paseito.52")
