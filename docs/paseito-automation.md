...
in iTerm2 so the human can supply credentials, resolve ambiguity, or stop the run.
...
semantic controller validates normalized history before starting its mechanical rebase. Codex then
classifies every feature as `carry_forward`, `adapt`, `upstream_complete`, or `blocked`. It may retire
a non-permanent feature only when upstream independently satisfies every invariant and executable
contract. A changelog or similarly named upstream feature is not proof.
