...
declare module "@getpaseo/plugin/desktop" {
  ...
  export type PluginMRAutomationField =
    | { key: string; type: "text" | "multiline" | "number" | "boolean" | "json"; label: string; required?: boolean; placeholder?: string }
    | { key: string; type: "select" | "multi-select"; label: string; required?: boolean; options: Array<{ value: string; label: string }> }
    | { key: string; type: "gitlab-user" | "gitlab-users"; label: string; required?: boolean };
  ...
  export interface PluginMRPredicateContribution {
    ...
    evaluate(input: { config: Record<string, unknown>; mergeRequest: PluginMRSnapshot }): PluginMRMatchState | Promise<PluginMRMatchState>;
  }
  ...
  export interface PluginMROperationContribution {
    ...
    fields: PluginMRAutomationField[];
    run(input: { config: Record<string, unknown>; mergeRequest: PluginMRSnapshot }): void | string | Promise<void | string>;
  }
