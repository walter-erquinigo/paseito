...
async function addInlineReviewComment(
  ...
  const gutter = page.locator('[data-testid^="diff-review-gutter-"]').nth(gutterIndex - 1);
...
test("expands a sent Review widget to show its comments", async ({ page }) => {
  ...
  try {
    ...
    await expect(page.getByTestId("diff-file-0")).toHaveAccessibleName(/^src\/review-me\.ts,/, {
      timeout: 30_000,
    });
