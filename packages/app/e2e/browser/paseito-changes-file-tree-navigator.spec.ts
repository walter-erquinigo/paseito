...
test("Changes pane navigates files from a responsive persistent right rail", async ({ page }) => {
  const workspace = await createNavigatorWorkspace();
  try {
    await page.addInitScript((storageKey) => {
      localStorage.setItem(storageKey, JSON.stringify({ openSupportingTabsInSidePanel: false }));
    }, APP_SETTINGS_KEY);
    await openFullChanges(page, workspace.id);
    ...
    await expect(panel.getByTestId(/^diff-file-\d+-body$/).filter({ visible: true })).toHaveCount(
      0,
    );
    ...
    const selectedPosition = await headerPositionInViewport(panel, TARGET_FILE_INDEX);
    expect(selectedPosition.offset).toBeGreaterThanOrEqual(0);
    expect(selectedPosition.offset).toBeLessThan(selectedPosition.viewportHeight);
    ...
    await expect(diffScroll).toBeFocused();
    await expect
      .poll(async () => (await headerPositionInViewport(panel, TARGET_FILE_INDEX)).offset)
      .toBe(selectedPosition.offset);
...
async function openFullChanges(page: Page, workspaceId: string): Promise<void> {
  await page.setViewportSize({ width: 2600, height: 900 });
  ...
  await expect(
    panel.getByTestId("diff-file-0-toggle").getByText("file-00.ts", { exact: true }),
  ).toBeVisible({
    timeout: 30_000,
  });
}

async function headerPositionInViewport(
  panel: ReturnType<Page["getByTestId"]>,
  index: number,
): Promise<{ offset: number; viewportHeight: number }> {
  ...
  if (!header || !scroll) throw new Error("Changed-file header is outside the diff viewport");
  return { offset: header.y - scroll.y, viewportHeight: scroll.height };
}
