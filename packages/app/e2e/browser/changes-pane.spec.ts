...
async function startReviewOnFirstChangedLine(
  ...
  await page.mouse.move(gutter.x + gutter.width / 2, gutter.y + gutter.height / 2);
...
async function firstReviewGutterBounds(
  ...
  const candidates = await page
    .locator('[data-testid^="diff-review-gutter-"]')
  ...
  const candidate = candidates.find(
    (bounds) =>
      bounds.height > 0 &&
      bounds.y + bounds.height > bodyBounds.y &&
      bounds.y < bodyBounds.y + bodyBounds.height &&
      (side !== "right" || bounds.x >= midpoint - 2),
  );
  if (!candidate) throw new Error(`No visible ${side} review gutter is available`);
  return candidate;
}
