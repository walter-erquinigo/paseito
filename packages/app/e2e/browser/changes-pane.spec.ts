...
test("Changes marks individual lines and a file as reviewed", async ({ page }) => {
  ...
  const fileReview = fileReviewCheckbox(page, "src/use-mounted-tab-set.ts");
...
test("the whole reviewable row reveals the gutter affordance and uses a text cursor", async ({
  ...
  await page.mouse.move(bodyBounds.x + bodyBounds.width - 32, gutter.y + gutter.height / 2);
...
test("line review controls stay in the fixed gutter across diff layouts", async ({ page }) => {
  ...
  const panel = page.getByTestId("working-diff-panel").filter({ visible: true });
  const reviewCheckboxes = panel.getByTestId(/^diff-line-review-/);
  await expect(reviewCheckboxes.first()).toHaveCSS("opacity", "1");
  await reviewCheckboxes.first().hover();
  await expect(reviewCheckboxes.first()).toHaveCSS("opacity", "1");
  const selectedReviewLine = panel.locator('[data-paseito-review-selected="true"]');
  const reviewFocusMarker = panel.getByTestId(/^diff-review-focus-/);
  ...
  await reviewCheckboxes.nth(2).click();
  await expect(fileReviewCheckbox(page)).toHaveAccessibleName("Mark file reviewed");
  ...
  const unifiedCheckbox = panel.getByTestId(/^diff-line-review-/).first();
  await expectCheckboxFixedWhileCodeScrolls(page, 0, unifiedCheckbox);

  const wrapLines = page.getByTestId("changes-toggle-wrap-lines").filter({ visible: true });
  const toggleLayout = page.getByTestId("changes-toggle-layout").filter({ visible: true });
  await wrapLines.click();
  await expectLineReviewControls(page, workspace.editedLineCount);
  await expect(page.getByTestId("diff-file-0-horizontal-scroll")).toHaveCount(0);

  await wrapLines.click();
  await toggleLayout.click();
  await expectLineReviewControls(page, workspace.editedLineCount);
  await expectCheckboxFixedWhileCodeScrolls(
    page,
    0,
    panel.getByTestId(/^diff-line-review-/).first(),
  );
});
...
test("Review menu marks, clears, and organizes every changed file", async ({ page }) => {
  ...
  const panel = page.getByTestId("working-diff-panel").filter({ visible: true });
  const reviewMenu = panel.getByTestId("changes-review-menu");
  const fileHeaders = panel.getByTestId(/^diff-file-\d+-toggle$/);
  const fileReviewControls = panel.getByRole("checkbox", { name: /^Mark file / });
  ...
  await page
    .getByTestId("changes-review-menu-content")
    .filter({ visible: true })
    .getByTestId("changes-review-mark-all")
    .click();
  ...
  await page
    .getByTestId("changes-review-menu-content")
    .filter({ visible: true })
    .getByTestId("changes-review-clear-all")
    .click();
  ...
  await page
    .getByTestId("changes-review-menu-content")
    .filter({ visible: true })
    .getByTestId("changes-review-organize")
    .click();
  for (const header of await fileHeaders.all()) {
...
test("Changes keyboard focus, full expansion, and source search share one review surface", async ({
  ...
  const selectedAgentTab = page.locator(
    '[data-testid^="workspace-tab-draft_"][aria-selected="true"]',
  );
  await expect(agentPrompt).toBeVisible();
  await expect(selectedAgentTab).toHaveCount(1);
  const agentTabTestId = await selectedAgentTab.getAttribute("data-testid");
  expect(agentTabTestId).not.toBeNull();
  const agentTab = page.getByTestId(agentTabTestId as string);
  await agentPrompt.focus();
  await expect(agentPrompt).toBeFocused();
  await page.getByTestId("workspace-tab-working_diff").filter({ visible: true }).first().click();
  const panel = page.getByTestId("working-diff-panel").filter({ visible: true });
  await expect(panel).toBeVisible();
  const commentScrollTop = await page
    .getByTestId("git-diff-scroll")
    .evaluate((element) => element.scrollTop);
  await clickReviewCommentGutter(
    page,
    panel.getByTestId(/^diff-review-gutter-(?!action-)/).first(),
  );
...
test("inline review comments retain focus beside an agent pane", async ({ page }) => {
  ...
  const launcher = page.getByTestId("workspace-new-tab-panel").filter({ visible: true });
  await expect(launcher).toBeVisible();
  await expect(launcher.getByRole("button", { name: "Agent", exact: true })).toBeFocused();
...
test("opening an inline comment preserves the viewport or reveals only the clipped editor", async ({
  ...
  const commentGutters = page.getByTestId(/^diff-review-gutter-(?!action-)/);
  const middleCommentGutter = commentGutters.nth(14);
  await middleCommentGutter.evaluate((element) => {
  ...
  const middleScrollTop = await page
    .getByTestId("git-diff-scroll")
    .evaluate((element) => element.scrollTop);
  await clickReviewCommentGutter(page, middleCommentGutter);
  await expect(page.getByTestId("inline-review-editor-input")).toBeFocused();
  await expect
    .poll(() => page.getByTestId("git-diff-scroll").evaluate((element) => element.scrollTop))
  ...
  const lowerCommentGutter = commentGutters.nth(27);
  await lowerCommentGutter.evaluate((element) => {
    let viewport = element.parentElement;
    while (viewport) {
      const style = getComputedStyle(viewport);
      if (
  ...
  const lowerScrollTop = await page
    .getByTestId("git-diff-scroll")
    .evaluate((element) => element.scrollTop);
  await clickReviewCommentGutter(page, lowerCommentGutter);
...
test("saved inline comments keep the fixed gutter aligned with code rows", async ({ page }) => {
  ...
  await clickReviewCommentGutter(page, targetButton);
  await page
  ...
  await clickReviewCommentGutter(page, secondTargetButton);
  await page
    .getByTestId("inline-review-editor-input")
...
test("Changes keeps pane-local navigation stable while other pane views open", async ({ page }) => {
  ...
  await configureDiffPresentation(page, {
    layout: "unified",
    wrapLines: false,
    inlineDiff: true,
  });
  await openWorkspaceCombinedChanges(page, workspace);
  ...
  await writeFile(path.join(workspace.repoPath, "src/use-mounted-tab-set.ts"), BEFORE);
  await expect(diffHeaderForPath(panel, "src/use-mounted-tab-set.ts")).toHaveCount(0, {
    timeout: 30_000,
  });
  await expect(diffHeaderForPath(panel, "src/zz-deleted.ts")).toBeVisible();
  await writeFile(path.join(workspace.repoPath, "src/use-mounted-tab-set.ts"), AFTER);
...
test("Changes toggles its optional tree rail without replacing the diff", async ({ page }) => {
  ...
  const optionsMenu = panel.getByTestId("changes-options-menu");
  await optionsMenu.click();
  let optionsContent = page.getByTestId("changes-options-menu-content").filter({ visible: true });
  await expect(optionsContent.getByTestId("changes-toggle-layout")).toBeVisible();
  await expect(optionsContent.getByTestId("changes-toggle-whitespace")).toContainText(
    "Hide whitespace",
  );
  await expect(optionsContent.getByTestId("changes-toggle-wrap-lines")).toContainText(
    "Wrap long lines",
  );
  await optionsContent.getByTestId("changes-toggle-whitespace").click();
  await optionsMenu.click();
  optionsContent = page.getByTestId("changes-options-menu-content").filter({ visible: true });
  await expect(optionsContent.getByTestId("changes-toggle-whitespace")).toContainText(
    "Show whitespace",
  );
  await page.keyboard.press("Escape");
  ...
  await panel.getByTestId("changes-toggle-tree").click();
  const tree = panel.getByTestId("changes-tree-rail");
...
async function expectFlatFileList(panel: Locator): Promise<void> {
  await expect(panel.getByTestId("changes-tree-rail")).toHaveCount(0);
  await expect(diffHeaderForPath(panel, "src/use-mounted-tab-set.ts")).toBeVisible();
}

async function expectLineReviewControls(
  page: Page,
  editedLineCount: number | undefined,
): Promise<void> {
  if (editedLineCount === undefined) throw new Error("Workspace has no edited-line count");
...
function fileReviewCheckbox(page: Page, filePath = "src/use-mounted-tab-set.ts"): Locator {
  const panel = page.getByTestId("working-diff-panel").filter({ visible: true });
  return diffHeaderForPath(panel, filePath).getByRole("checkbox", { name: /^Mark file / });
}
...
async function clickReviewCommentGutter(page: Page, gutter: Locator): Promise<void> {
  const bounds = await gutter.boundingBox();
  if (!bounds) throw new Error("Review comment gutter is unavailable");
  await page.mouse.move(bounds.x + Math.max(1, bounds.width - 2), bounds.y + bounds.height / 2);
    ...
    ({ preferencesKey, presentation }) => {
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          inlineDiff: presentation.inlineDiff ?? false,
        }),
      );
    },
...
async function openWorkspaceCombinedChanges(page: Page, workspace: DirtyWorkspace): Promise<void> {
  await page.setViewportSize({ width: 1400, height: 900 });
  await page.goto(buildHostWorkspaceRoute(getServerId(), workspace.id));
  await waitForWorkspaceTabsVisible(page);
  await page.getByTestId("workspace-explorer-toggle").first().click();
...
async function startReviewOnFirstChangedLine(
  ...
  const bodyBounds = await body.boundingBox();
  if (!bodyBounds) throw new Error("Expanded diff body has no bounds");
  const gutter = await firstReviewGutterBounds(page, bodyBounds, side);
  await page.mouse.move(gutter.x + Math.max(1, gutter.width - 2), gutter.y + gutter.height / 2);
  await page.getByRole("button", { name: "Add review comment" }).click();
...
async function firstReviewGutterBounds(
  page: Page,
  ...
  const bounds = await gutters.evaluateAll((elements) =>
    elements.map((element) => {
      const rect = element.getBoundingClientRect();
      return { x: rect.x, y: rect.y, width: rect.width, height: rect.height };
    }),
  );
  const midpoint = bodyBounds.x + bodyBounds.width / 2;
  const candidates = bounds
    .filter(
      (rect) =>
        ...
        rect.height > 0 &&
        rect.y >= bodyBounds.y &&
        rect.y < bodyBounds.y + bodyBounds.height &&
        (side === "right" ? rect.x >= midpoint : rect.x < midpoint),
    )
    .sort((left, right) => left.y - right.y);
  const gutter = candidates[0];
  if (!gutter) throw new Error(`No visible ${side} review gutter is available`);
  return gutter;
}
