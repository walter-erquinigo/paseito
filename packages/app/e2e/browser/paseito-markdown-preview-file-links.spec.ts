...
test("Markdown preview locations reuse an existing Changes tab when its current line is available", async ({
  ...
  try {
    ...
    await expect(panel.getByText("long.ts", { exact: true })).toBeVisible({ timeout: 30_000 });
    await openFileExplorer(page);
    await openFileFromExplorer(page, "notes.md");
    ...
    await expect(panel.getByTestId("git-diff-canvas-root")).toHaveAttribute(
      "data-paseito-diff-focus-path",
      "long.ts",
    );
    await expect(panel.getByTestId("git-diff-canvas-root")).toHaveAttribute(
      "data-paseito-diff-focus-line-start",
      "40",
    );
    const selectedColumnLine = diffScroll.locator(
    ...
    await expect(panel.getByTestId("diff-file-0-body")).toBeHidden();
