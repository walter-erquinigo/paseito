...
import { openChangesPanel, openFilesPanel } from "../support/helpers/workspace-tabs";
...
test("Command+Enter opens changed files in Changes and keeps missing files in the dialog", async ({
  ...
  try {
    ...
    const mainPane = page.getByTestId("workspace-pane-main").filter({ visible: true });
    ...
    const sidePanel = page
      .locator(
        '[data-testid^="workspace-pane-"]:not([data-testid="workspace-pane-main"]):not([data-testid="workspace-pane-explorer"])',
      )
      .filter({ has: page.getByTestId("working-diff-panel"), visible: true });
    const changes = sidePanel.getByTestId("working-diff-panel");
    await expect(changes).toBeVisible();
    await expect(changes.getByTestId("git-diff-canvas-root")).toHaveAttribute(
      "data-paseito-diff-focus-path",
      changedPath,
    );
    await expect(mainPane.getByTestId("working-diff-panel")).toHaveCount(0);
