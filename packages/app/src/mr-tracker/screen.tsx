...
import { MRAutomationPanel } from "./automation-panel";
import { AdaptiveModalSheet } from "@/components/adaptive-modal-sheet";
import type { MRAutomationRenderedAction } from "./automation-types";
import type { MRAutomationRule, MRAutomationViewState } from "./automation-types";
...
function automationVisibility(
  ...
  rulesOpen: boolean,
  hasToken: boolean,
  rulesInSheet: boolean,
) {
  const available = tab === "my_mrs" && hasToken;
  return {
    toggle: tab === "my_mrs" && !rulesOpen,
    panel: available && rulesOpen && !rulesInSheet,
    sheet: available && rulesOpen && rulesInSheet,
  };
}

function automationActionsDisabled(pending: boolean, pendingId: string | null): boolean {
  return pending || pendingId !== null;
}

async function runAutomationAction(input: {
  mergeRequestId: string;
  action: MRAutomationRenderedAction;
  t: TFunction;
  run(id: string, action: () => Promise<unknown>): Promise<void>;
  execute(mergeRequestId: string, ruleId: string, outcomeId: string): Promise<unknown>;
}): Promise<void> {
  const { action } = input;
  if (action.kind === "link") {
    if (action.href) await openExternalUrl(action.href);
    return;
  }
  if (action.requireConfirmation) {
    const confirmed = await confirmDialog({
      title: action.label,
      message: input.t("mrTracker.automation.confirmMessage", { label: action.label }),
      confirmLabel: action.label,
      cancelLabel: input.t("common.actions.cancel"),
    });
    if (!confirmed) return;
  }
  await input.run(action.id, () =>
    input.execute(input.mergeRequestId, action.ruleId, action.outcomeId),
  );
}
...
// eslint-disable-next-line complexity -- This screen coordinates mutually exclusive loading, setup, empty, list, and adaptive automation surfaces.
export function MRTrackerScreen({ tab, focusId, focusRevision }: MRTrackerScreenProps) {
  ...
  const [rulesOpen, setRulesOpen] = useState(tab === "my_mrs");
  useEffect(() => {
    if (tab === "my_mrs") setRulesOpen(true);
  }, [tab]);
  const { width: windowWidth } = useWindowDimensions();
  const rulesInSheet = windowWidth < 1_000;
  const rulesVisibility = automationVisibility(
    tab,
    rulesOpen,
    state?.hasToken === true,
    rulesInSheet,
  ...
  const handleAutomationAction = useCallback(
    async (mergeRequestId: string, action: MRAutomationRenderedAction) =>
      runAutomationAction({
        mergeRequestId,
        action,
        t,
    ...
    [executeAutomationAction, run, t],
  );
  const handleOpenRules = useCallback(() => setRulesOpen(true), []);
  const handleCloseRules = useCallback(() => setRulesOpen(false), []);
  const handleSaveAutomationRules = useCallback(
    async (rules: MRAutomationRule[]) => {
      await replaceAutomationRules(rules);
    },
    [replaceAutomationRules],
  );
  const rulesSheetHeader = useMemo(() => ({ title: t("mrTracker.automation.title") }), [t]);
  const headerActions = useMemo(
    () => (
      <View style={styles.headerActions}>
        ...
        <AutomationRulesToggle visible={rulesVisibility.toggle} onPress={handleOpenRules} />
        <Button
          size="xs"
          variant="ghost"
          leftIcon={RefreshCw}
          ...
          <View key={stack.id} style={styles.stackCard}>
            ...
            {stack.entries.map(({ mergeRequest, depth, context }) => (
              <MRRow
                ...
                automationActions={state.automation.actionsByMergeRequestId[mergeRequest.id] ?? []}
                automationPendingId={pendingAction}
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.contentFrame}>
        ...
        <AutomationRulesSurface
          location="panel"
          visible={rulesVisibility.panel}
          automation={state?.automation}
          sheetHeader={rulesSheetHeader}
          onClose={handleCloseRules}
          onSave={handleSaveAutomationRules}
          onPreview={previewAutomationRule}
        />
      </View>
      <AutomationRulesSurface
        location="sheet"
        visible={rulesVisibility.sheet}
        automation={state?.automation}
        sheetHeader={rulesSheetHeader}
        onClose={handleCloseRules}
        onSave={handleSaveAutomationRules}
...
function AutomationRulesToggle({ visible, onPress }: { visible: boolean; onPress(): void }) {
  const { t } = useTranslation();
  if (!visible) return null;
  return (
    <Button size="xs" variant="ghost" leftIcon={Workflow} onPress={onPress}>
      {t("mrTracker.automation.rules")}
    </Button>
  );
}

function AutomationRulesSurface({
  location,
  visible,
  automation,
  sheetHeader,
  ...
  return (
    <AdaptiveModalSheet visible header={sheetHeader} onClose={onClose} testID="mr-automation-sheet">
      {panel}
    </AdaptiveModalSheet>
  );
}
...
function MRRow({
  ...
  activityUserIds: ReadonlySet<number>;
  automationActions: MRAutomationRenderedAction[];
  automationPendingId: string | null;
  onAutomationAction: (mergeRequestId: string, action: MRAutomationRenderedAction) => Promise<void>;
}) {
  const { t } = useTranslation();
  ...
  return (
    <View style={rowStyle} testID={`mr-row-${value.id}`}>
      ...
      <View style={rowContentStyle}>
        ...
        <MRAutomationActions
          mergeRequestId={value.id}
          actions={automationActions}
          disabled={automationActionsDisabled(pending, automationPendingId)}
          pendingId={automationPendingId}
...
function MRAutomationActions({
  ...
  mergeRequestId: string;
  actions: MRAutomationRenderedAction[];
  disabled: boolean;
  pendingId: string | null;
  onAction(mergeRequestId: string, action: MRAutomationRenderedAction): Promise<void>;
}) {
  if (!actions.length) return null;
  return (
    <View style={styles.automationActions}>
      {actions.map((action) => (
        <MRAutomationActionButton
          key={action.id}
...
function MRAutomationActionButton({
  mergeRequestId,
  action,
  disabled,
  pending,
  ...
  const handlePress = useCallback(
    () => void onAction(mergeRequestId, action),
    [action, mergeRequestId, onAction],
  );
  return (
    <Button
      size="xs"
      variant={action.kind === "link" ? "outline" : "secondary"}
      leftIcon={action.kind === "link" ? ExternalLink : undefined}
      disabled={disabled}
      loading={pending}
      onPress={handlePress}
    >
...
const styles = StyleSheet.create((theme) => ({
  ...
  contentFrame: { flex: 1, flexDirection: "row", minHeight: 0 },
  mrContent: { flex: 1, minWidth: 0 },
  headerActions: {
    flexDirection: "row",
    alignItems: "center",
    gap: theme.spacing[1],
