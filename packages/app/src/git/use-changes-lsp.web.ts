...
import type { WorkspaceLspHover, WorkspaceLspLocation } from "@getpaseo/protocol/messages";
...
export function useChangesLsp(input: {
  ...
  const hover = useCallback(
    async (filePath: string, lineNumber: number, column: number) => {
      ...
      const hoverResult = await session.hover({ line: lineNumber - 1, character: column - 1 });
      return hoverResult ? hoverText(hoverResult) : null;
    },
    [controller, enabled],
  );
