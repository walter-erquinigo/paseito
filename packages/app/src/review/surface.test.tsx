...
function suggestion(overrides: Partial<ReviewDraftSuggestion> = {}): ReviewDraftSuggestion {
  return {
    id: "suggestion-1",
    filePath: "src/example.ts",
    startLine: 2,
    endLine: 2,
    originalLines: ["const value = next;"],
    replacement: "const value = changed;",
    note: "Use the new value.",
...
describe("useInlineReviewController", () => {
  beforeEach(() => {
    useReviewDraftStore.setState({ drafts: {}, suggestions: {} });
  });
...
describe("InlineReviewThread", () => {
  ...
  it("aligns saved suggestions after the diff gutter", () => {
    const reviewTarget = target({ sourceRevision: "revision-1" });
    const draftSuggestion = suggestion();
    const actions = buildReviewActions({
      suggestionsByTarget: new Map([[reviewTarget.key, [draftSuggestion]]]),
    });
