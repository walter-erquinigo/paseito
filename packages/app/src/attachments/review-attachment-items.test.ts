...
describe("review attachment list items", () => {
  it("builds readable locations for added and removed-line comments", () => {
    expect(buildReviewAttachmentListItems(attachment).slice(0, 2)).toEqual([
      {
  ...
  it("includes code changes in the visible entry count and list", () => {
    expect(getReviewAttachmentEntryCount(attachment)).toBe(3);
    expect(buildReviewAttachmentListItems(attachment)[2]).toEqual({
