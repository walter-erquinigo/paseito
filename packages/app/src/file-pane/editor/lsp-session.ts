...
export class EditorLspSession {
  ...
  private snapshot: EditorLspSnapshot = {
    status: "connecting",
    error: null,
    provider: null,
  };
  ...
  async open(content: string): Promise<void> {
    ...
    this.version += 1;
    this.publish({ status: "connecting", error: null, provider: null });
    this.opening = (async () => {
      try {
        ...
        if (this.disposed) {
          ...
          const provider =
            result.kind === "ack" &&
            "provider" in result &&
            (result.provider === "lens" || result.provider === "clangd")
              ? result.provider
              : null;
          this.publish({ status: "ready", error: null, provider });
        }
      } catch (error) {
        this.disable(error);
      }
  ...
  matchesContent(content: string): boolean {
    return !this.disposed && this.content === content;
  }
  ...
  private publish(snapshot: EditorLspSnapshot): void {
