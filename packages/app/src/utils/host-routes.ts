...
export function buildMRTrackerRoute(
  tab: MRTrackerTabSlug,
  mergeRequestId?: string,
  focusRevision?: string | number,
) {
  const route = `/mrs/${tab}` as const;
  if (!mergeRequestId) return route;
  const params = new URLSearchParams({ mr: mergeRequestId });
  if (focusRevision !== undefined) params.set("focus", String(focusRevision));
  return `${route}?${params.toString()}` as const;
}
