...
import { MRAutomationEngine, type MRAutomationExecutor } from "./automation-engine.js";
import {
...
export interface GitLabTrackerClient {
  ...
  namedPipelines?(projectRef: string | number, sha: string): Promise<MRNamedPipeline[]>;
  postComment?(projectRef: string | number, iid: number, body: string): Promise<void>;
  addReviewers?(
    projectRef: string | number,
    iid: number,
...
export interface MRTrackerServiceOptions {
  ...
  now?: () => Date;
  automationEngine?: MRAutomationEngine;
}

function emptyAutomationViewState(engine?: MRAutomationEngine): MRAutomationViewState {
  return {
    rules: [],
    predicates: engine?.predicateDescriptors() ?? structuredClone(BUILTIN_MR_AUTOMATION_PREDICATES),
    operations: engine?.operationDescriptors() ?? structuredClone(BUILTIN_MR_AUTOMATION_OPERATIONS),
    actionsByMergeRequestId: {},
    receipts: [],
    errors: [],
  };
}

function conditionUsesPipeline(condition: MRAutomationRule["condition"]): boolean {
  if (condition.kind === "predicate")
    return condition.contributionId.startsWith("gitlab.pipeline.");
  if (condition.kind === "not") return conditionUsesPipeline(condition.child);
  return condition.children.some(conditionUsesPipeline);
}
...
export class MRTrackerService {
  ...
  private state: MRTrackerViewState = {
    ...
    automation: emptyAutomationViewState(),
  };
  ...
  private automationContexts = new Map<string, MRAutomationEvaluationContext>();
  ...
  async start(): Promise<void> {
    ...
    const automationState = await this.options.automationEngine?.load();
    this.setState({
      ...
      automation: automationState
        ? {
            ...emptyAutomationViewState(this.options.automationEngine),
            rules: automationState.rules,
            receipts: automationState.receipts,
          }
        : emptyAutomationViewState(this.options.automationEngine),
    });
    this.installTimer();
  ...
  async replaceAutomationRules(rules: MRAutomationRule[]): Promise<MRTrackerViewState> {
    await this.start();
    if (!this.options.automationEngine) throw new Error("MR automations are unavailable.");
    const automation = await this.options.automationEngine.replaceRules(rules);
    this.setState({
      ...this.state,
      automation: {
  ...
  async executeAutomationAction(
    ...
    await this.start();
    const engine = this.options.automationEngine;
    if (!engine) throw new Error("MR automations are unavailable.");
    await this.refresh();
    const context = this.automationContexts.get(mergeRequestId);
    if (!context) throw new Error("This merge request is unavailable.");
    const token = await this.requireToken();
    const client = this.createClient(this.settings, token);
    await engine.executeManual(ruleId, outcomeId, context, this.automationExecutor(client));
    return await this.refresh();
  }

  async previewAutomationRule(rule: MRAutomationRule): Promise<MRAutomationPreviewResult[]> {
    await this.start();
    if (!this.options.automationEngine) throw new Error("MR automations are unavailable.");
    await this.refresh();
    if (conditionUsesPipeline(rule.condition)) await this.loadPreviewPipelines();
    return await this.options.automationEngine.preview(rule, [...this.automationContexts.values()]);
  }

  private async loadPreviewPipelines(): Promise<void> {
    const token = await this.requireToken();
    const client = this.createClient(this.settings, token);
    const loadNamedPipelines = client.namedPipelines?.bind(client);
    if (!loadNamedPipelines) return;
    await Promise.all(
      [...this.automationContexts].map(async ([id, context]) => {
        if (!context.mergeRequest.isOwned) return;
        try {
          const namedPipelines = await loadNamedPipelines(
            context.mergeRequest.projectId,
            context.mergeRequest.sourceSha,
          );
          this.automationContexts.set(id, { ...context, namedPipelines });
        } catch {
          this.automationContexts.set(id, { ...context, namedPipelines: null });
        }
      }),
    );
  }

  async resolveAutomationForUrl(
    input: string,
    options: { refresh?: boolean } = {},
  ): Promise<{
    mergeRequestId: string;
    actions: MRAutomationViewState["actionsByMergeRequestId"][string];
  }> {
    const navigation = await this.resolveNavigation(input);
    const state = options.refresh ? await this.refresh() : this.state;
    return {
      mergeRequestId: navigation.mergeRequestId,
      actions: state.automation.actionsByMergeRequestId[navigation.mergeRequestId] ?? [],
    };
  }
  ...
  private async performRefresh(): Promise<MRTrackerViewState> {
    ...
    try {
      ...
      const automationActions = await this.evaluateAutomations(
        uniqueSnapshots,
        previousById,
        client,
        collection.complete,
      ...
      this.setState({
        ...
        automation: {
          ...this.state.automation,
          predicates: this.options.automationEngine?.predicateDescriptors() ?? [],
          operations: this.options.automationEngine?.operationDescriptors() ?? [],
          actionsByMergeRequestId: automationActions,
          receipts:
            (await this.options.automationEngine?.load())?.receipts ??
            this.state.automation.receipts,
        },
      });
  ...
  private async hydrate(
    ...
    const snapshot: MergeRequestSnapshot = {
      id,
      projectId: value.project_id,
      projectPath: projectPathFromMergeRequestUrl(value.web_url),
      iid: value.iid,
    ...
    const namedPipelines = await this.loadNamedPipelines(client, snapshot, onPartialError);
    this.automationContexts.set(id, {
  ...
  private async loadNamedPipelines(
    client: GitLabTrackerClient,
    snapshot: MergeRequestSnapshot,
    onPartialError: (area: string, error: unknown) => void,
  ): Promise<MRNamedPipeline[] | null> {
    const needed = this.state.automation.rules.some(
      (rule) => rule.enabled && conditionUsesPipeline(rule.condition),
    );
    if (!this.options.automationEngine || !needed) return null;
    if (!client.namedPipelines) {
      onPartialError("named pipelines", new Error("Named pipeline lookup is unavailable."));
      return null;
    }
    try {
      return await client.namedPipelines(snapshot.projectId, snapshot.sourceSha);
    } catch (error) {
      onPartialError("named pipelines", error);
      return null;
    }
  }
  ...
  private async evaluateAutomations(
    ...
    previousById: ReadonlyMap<string, MergeRequestSnapshot>,
    client: GitLabTrackerClient,
    complete: boolean,
  ) {
    const engine = this.options.automationEngine;
    ...
    const contexts = snapshots.map((snapshot) => {
      const cached = this.automationContexts.get(snapshot.id);
      return {
        mergeRequest: snapshot,
        namedPipelines: cached?.namedPipelines ?? null,
        isNew: !previousById.has(snapshot.id),
      } satisfies MRAutomationEvaluationContext;
    });
    this.automationContexts = new Map(
      contexts.map((context) => [context.mergeRequest.id, context]),
    );
    return await engine.evaluate(contexts, complete, this.automationExecutor(client));
  }

  private automationExecutor(client: GitLabTrackerClient): MRAutomationExecutor {
    const resolve = (mergeRequestId: string) => {
      const snapshot = this.automationContexts.get(mergeRequestId)?.mergeRequest;
      if (!snapshot) throw new Error("This merge request is unavailable.");
      return snapshot;
    };
    return {
      postComment: async (mergeRequestId, body) => {
        if (!client.postComment) throw new Error("This GitLab client cannot post comments.");
        const snapshot = resolve(mergeRequestId);
        await client.postComment(snapshot.projectId, snapshot.iid, body);
      },
      addReviewers: async (mergeRequestId, usernames) => {
        if (!client.addReviewers) throw new Error("This GitLab client cannot add reviewers.");
        const snapshot = resolve(mergeRequestId);
        await client.addReviewers(snapshot.projectId, snapshot.iid, usernames);
      },
    };
  }
