...
import type { MRNamedPipeline } from "./automation-types.js";
...
export class GitLabClient {
  ...
  async namedPipelines(projectRef: string | number, sha: string): Promise<MRNamedPipeline[]> {
    if (!sha.trim()) return [];
    const values = await this.getAll<GitLabApiPipeline>(
      `projects/${encodeURIComponent(String(projectRef))}/pipelines`,
      { sha },
    );
    return values
      .flatMap((value): MRNamedPipeline[] => {
        if (typeof value.id !== "number" || !value.name || !value.status) return [];
        return [
          {
            id: value.id,
            name: value.name,
  ...
  async postComment(projectRef: string | number, iid: number, body: string): Promise<void> {
    if (!body.trim()) throw new Error("Comment body is required.");
    await this.request(
      `projects/${encodeURIComponent(String(projectRef))}/merge_requests/${iid}/notes`,
      {},
  ...
  async addReviewers(
    ...
    const requested = [...new Set(usernames.map((value) => value.trim()).filter(Boolean))];
    if (!requested.length) throw new Error("At least one reviewer is required.");
    const [mergeRequest, ...resolved] = await Promise.all([
      this.mergeRequest(projectRef, iid),
      ...requested.map((username) => this.exactUser(username)),
    ]);
    const missing = requested.filter((_username, index) => !resolved[index]);
    if (missing.length) throw new Error(`GitLab users not found: ${missing.join(", ")}.`);
    const reviewerIds = [
      ...new Set([
        ...(mergeRequest.reviewers ?? []).map((user) => user.id),
        ...resolved.flatMap((user) => (user ? [user.id] : [])),
      ]),
    ];
    if (reviewerIds.length === (mergeRequest.reviewers ?? []).length) return;
    await this.request(
      `projects/${encodeURIComponent(String(projectRef))}/merge_requests/${iid}`,
      {},
      { method: "PUT", body: { reviewer_ids: reviewerIds } },
    );
  }
