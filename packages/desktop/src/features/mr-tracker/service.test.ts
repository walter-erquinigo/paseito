...
describe("MRTrackerService", () => {
  ...
  it("resolves a GitLab URL, tracks a missing MR locally, and chooses its ownership tab", async () => {
    ...
    const queriesAfterNavigation = authorQueries;
    await expect(service.resolveAutomationForUrl(value.web_url)).resolves.toMatchObject({
      mergeRequestId: "10:17",
      actions: [],
    });
    expect(authorQueries).toBe(queriesAfterNavigation);
    await service.resolveAutomationForUrl(value.web_url, { refresh: true });
    expect(authorQueries).toBe(queriesAfterNavigation + 1);
    await expect(
