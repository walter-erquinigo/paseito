...
import type { MRAutomationViewState } from "../../app/src/mr-tracker/automation-types";
...
function automationState(): MRAutomationViewState {
  ...
  return {
    ...
    predicates: [
      {
        ...
        title: "Merge request is approved",
        description: "Matches an approved merge request.",
        fields: [],
      },
      {
    ...
    operations: [
      ...
      },
      {
        ...
        title: "Open pipeline",
        description: "Opens the matching pipeline.",
        kind: "link",
    ...
    actionsByMergeRequestId: {
      [MR_ID]: [
        {
          id: "notify-ready:outcome-ready",
          ruleId: "notify-ready",
          outcomeId: "outcome-ready",
          label: "Notify team",
          kind: "button",
          requireConfirmation: true,
          href: null,
        ...
        {
          id: "inspect-pipeline:outcome-pipeline",
          ruleId: "inspect-pipeline",
          outcomeId: "outcome-pipeline",
          label: "Open pipeline",
          kind: "link",
          requireConfirmation: false,
          href: "https://gitlab.example.com/example/constellation/-/pipelines/99",
        },
      ],
    },
    receipts: [
...
async function getCapturedDialogMessage(page: Page): Promise<string | undefined> {
  return await page.evaluate(() => window.__capturedDialogCall?.message);
}
...
test.describe("MR tracker row interactions", () => {
  ...
  test("renders and operates the MR automation rail and narrow-screen sheet", async ({
    page,
    withWorkspace,
  }) => {
    const state = trackerState();
    await installDesktopRuntime(page, {
      ...
      commandResponses: {
        get_mr_tracker_state: state,
        execute_mr_automation_action: state,
        replace_mr_automation_rules: state,
        preview_mr_automation_rule: [{ mergeRequestId: MR_ID, state: "match" }],
      },
    });
    await page.setViewportSize({ width: 1800, height: 1000 });
    const workspace = await withWorkspace({ prefix: "mr-tracker-automation-" });
    await workspace.navigateTo();
    await page.getByTestId("mr-sidebar-my_mrs").click();

    const panel = page.getByTestId("mr-automation-panel");
    await expect(panel).toBeVisible();
    await expect(panel).toContainText("Notify when ready");
    await panel.getByRole("switch", { name: "Notify when ready enabled", exact: true }).click();
    await expect(page.getByTestId("mr-automation-editor")).toHaveCount(0);
    await expect
      .poll(() => getLastDesktopInvocationArgs(page, "replace_mr_automation_rules"))
      .toMatchObject({ rules: [{ id: "notify-ready", enabled: false }] });
    await expect(page.getByRole("button", { name: "Notify team", exact: true })).toBeVisible();
    ...
    await expect.poll(() => getOpenUrlCount(page)).toBe(1);
    expect((await getOpenUrls(page))[0]).toBe(
      "https://gitlab.example.com/example/constellation/-/pipelines/99",
    );

    await page.getByRole("button", { name: "Notify team", exact: true }).click();
    await expect
      .poll(() => getLastDesktopInvocationArgs(page, "execute_mr_automation_action"))
      .toEqual({
        mergeRequestId: MR_ID,
        ruleId: "notify-ready",
        outcomeId: "outcome-ready",
      });
    await expect.poll(() => getCapturedDialogMessage(page)).toContain("Notify team");

    await panel.getByText("Activity", { exact: true }).click();
    await expect(panel).toContainText("succeeded");
    await expect(panel).toContainText(MR_ID);
    await panel.getByRole("button", { name: "Collapse rules", exact: true }).click();
    await expect(panel).toHaveCount(0);
    await page.getByRole("button", { name: "Rules", exact: true }).click();
    await expect(page.getByTestId("mr-automation-panel")).toBeVisible();

    await page.getByTestId("mr-automation-rule-notify-ready").click();
    await expect(page.getByTestId("mr-automation-editor")).toContainText("Post comment");
    await expect(page.getByTestId("mr-automation-editor")).toContainText("READY_FOR_REVIEW");

    await page.setViewportSize({ width: 760, height: 900 });
    await expect(page.getByTestId("mr-automation-sheet")).toBeVisible();
    const overflow = await page.evaluate(() => document.documentElement.scrollWidth - innerWidth);
    expect(overflow).toBeLessThanOrEqual(0);
  });
});
