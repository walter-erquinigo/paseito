...
test("startAgentRun only steers an active run when explicitly requested", async () => {
  ...
  const steerOrReplaceSpy = vi
    .spyOn(manager, "steerOrReplaceActiveTurn")
    .mockResolvedValue({ status: "replaced", iterator: steeringEvents });
  ...
  expect(steerOrReplaceSpy).not.toHaveBeenCalled();
  ...
  expect(steerOrReplaceSpy).toHaveBeenCalledWith(snapshot.id, "new direction", undefined);
