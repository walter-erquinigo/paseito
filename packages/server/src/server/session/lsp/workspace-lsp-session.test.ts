...
describe("WorkspaceLspSession", () => {
  test("identifies the provider that opens the document", async () => {
    const cwd = workspace();
    ...
    expect(logger.info).toHaveBeenCalledWith(
      { workspace: cwd, provider: "clangd" },
      "workspace_lsp_backend_selected",
    );
  });

  test("logs and returns the complete backend startup failure", async () => {
    const cwd = workspace();
    const emitted: SessionOutboundMessage[] = [];
    const logger = { info: vi.fn(), warn: vi.fn() };
    const failure = new Error("spawn clangd ENOENT");
    const session = new WorkspaceLspSession(
      { emit: (message) => emitted.push(message) },
