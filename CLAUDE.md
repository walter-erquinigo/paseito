...
- **NEVER run the full test suite locally.** The test suites are heavy and will freeze the machine, especially if multiple agents run them in parallel. Rules:
  ...
  - Paseito does not run GitHub Actions. Use focused local coverage and the required repository checks; do not substitute a full local suite.
...
- `independent-desktop-identity` — Keep Paseito independently installable beside Paseo with distinct app, daemon, CLI, storage, protocol and visual identities.
  - Fix: Keep the desktop bundle, daemon, CLI, storage, URL scheme, local installer and artwork independent from Paseo.
...
- `local-semantic-maintenance` — Use human-supervised weekly Codex reconciliation, fail-closed local Apple Silicon verification and installation, migration and local daily reporting.
  ...
  - Fix: Verify deferred browser contracts, the local Apple Silicon package and rollback-safe local installation before updating the branch.
  - Fix: Keep weekly overlap decisions interactive and all build and installation work local.
  - Fix: Open weekly work in iTerm2 without launchd-owned AppleEvents.
  - Fix: Reject post-reconciliation candidates whose rebased history is not normalized before independent review or branch update.
  - Fix: Resume from an already-rebased source branch only after validating normalized history against the exact peeled stable upstream commit.
  - Fix: Bind explicit human-approved feature classifications into reconciliation and reject missing, duplicate, unknown or reclassified decisions.
  - Fix: Keep the next prerelease revision above every visible Paseito revision already assigned to the target upstream base.
  - Fix: Use GitHub only to fetch source and force-push the normalized Paseito branch with a lease; never dispatch Actions or publish releases, tags, provenance or assets.
